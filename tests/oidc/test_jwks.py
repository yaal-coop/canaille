...
def test_deterministic_jwk_generation_with_seed():
    ...
    jwk1 = make_default_jwk(test_seed)
    jwk2 = make_default_jwk(test_seed)
    jwk3 = make_default_jwk(test_seed)
    ...
    assert len(jwk1) == 1
    assert "kid" in jwk1[0]
    assert "kty" in jwk1[0]
    assert jwk1[0]["kty"] == "OKP"
    assert jwk1[0]["crv"] == "Ed25519"

    jwk4 = make_default_jwk("different-seed-456")

    assert jwk1 != jwk4

    jwk5 = make_default_jwk("different-seed-456")
    assert jwk4 == jwk5


def test_random_jwk_generation_without_seed():
    """Test that without seed, JWKs are randomly generated."""
    jwk1 = make_default_jwk(None)
    jwk2 = make_default_jwk(None)
