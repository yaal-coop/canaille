...
def ldap_connection(config):
    ...
    conn.set_option(ldap.OPT_NETWORK_TIMEOUT, config["CANAILLE_LDAP"].get("TIMEOUT"))
    conn.simple_bind_s(
...
class Backend(BaseBackend):
    ...
    def connection(self):
        ...
        try:
            ...
            self._connection.set_option(
                ...
                self.config["CANAILLE_LDAP"].get("TIMEOUT"),
            )
    ...
    def login_placeholder(cls):
        user_filter = current_app.config["CANAILLE_LDAP"].get(
...
def setup_ldap_models(config):
    ...
    models.User.rdn_attribute = config["CANAILLE_LDAP"].get(
        "USER_RDN", models.User.DEFAULT_RDN
    )
    object_class = config["CANAILLE_LDAP"].get(
        "USER_CLASS", models.User.DEFAULT_OBJECT_CLASS
    )
    models.User.ldap_object_class = listify(object_class)

    group_base = (
        config["CANAILLE_LDAP"]
        .get("GROUP_BASE", "")
        .replace(f',{config["CANAILLE_LDAP"]["ROOT_DN"]}', "")
    )
    models.Group.base = group_base or None
    models.Group.rdn_attribute = config["CANAILLE_LDAP"].get(
