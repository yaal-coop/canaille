...
def locale_selector():
    ...
    if current_app.config["CANAILLE"].get("LANGUAGE"):
        return current_app.config["CANAILLE"].get("LANGUAGE")
...
def timezone_selector():
    ...
    try:
        return pytz.timezone(current_app.config["CANAILLE"].get("TIMEZONE"))
