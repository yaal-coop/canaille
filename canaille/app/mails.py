...
def logo():
    logo_url = current_app.config["CANAILLE"].get("LOGO")
    if not logo_url:
...
def send_email(subject, recipient, text, html, attachements=None):
    ...
    name = current_app.config["CANAILLE"].get("NAME", "Canaille")
    address = current_app.config["CANAILLE"]["SMTP"].get("FROM_ADDR")
    ...
    try:
        ...
        with connection_func(
            ...
            if current_app.config["CANAILLE"]["SMTP"]["TLS"]:
                smtp.starttls()
            if current_app.config["CANAILLE"]["SMTP"].get("LOGIN"):
                smtp.login(
                    user=current_app.config["CANAILLE"]["SMTP"].get("LOGIN"),
                    password=current_app.config["CANAILLE"]["SMTP"].get("PASSWORD"),
                )
