...
def validate(config, validate_remote=False):
    ...
    validate_theme(config.get("CANAILLE"))
    ...
    validate_smtp_configuration(config.get("CANAILLE", {}).get("SMTP"))
...
def validate_keypair(config):
    if (
        config
        and config.get("JWT")
        and not config["JWT"].get("PUBLIC_KEY")
    ...
    if (
        config
        and config.get("JWT")
        and not config["JWT"].get("PRIVATE_KEY")
        and not current_app.debug
    ):
...
def validate_theme(config):
    if not config or not config.get("THEME"):
        return
