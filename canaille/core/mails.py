...
def send_password_reset_mail(user, mail):
    ...
    subject = _("Password reset on {website_name}").format(
        website_name=current_app.config["CANAILLE"].get("NAME", base_url)
    )
    text_body = render_template(
        "mails/reset.txt",
        site_name=current_app.config["CANAILLE"].get("NAME", base_url),
        site_url=base_url,
        reset_url=reset_url,
    )
    html_body = render_template(
...
def send_registration_mail(email, registration_url):
    ...
    subject = _("Continue your registration on {website_name}").format(
        website_name=current_app.config["CANAILLE"].get("NAME", registration_url)
    )
    text_body = render_template(
