...
- **A nonce is required by default.** Canaille enables :attr:`~canaille.oidc.configuration.OIDCSettings.REQUIRE_NONCE`. This is the server default for clients. Administrators can override it for an individual client in the application settings: use the server default, require a nonce, or do not require a nonce. The latter still accepts and validates a nonce when one is supplied; it does not reject nonce-bearing requests. Disable the requirement only for clients that cannot support it, and prefer fixing the client when possible.
